# mammary_pretumoral_evolution
This folder contains the code to reproduce the analysis from *Laisné, Schwartz 2026* paper. 

TO DO 

## Scripts
```
.
├── 0_Utils.R
├── 1.1.1_MiniBulkLineage_PreProcess.Rmd
├── 1.1.1_MiniBulkRNA_analysis.Rmd
├── 1.1.2_MiniBulkRNA_Tumor_vs_Juxta.Rmd
├── 1.2_MiniBulkRNA_Figures.Rmd
├── 2.2_MiniBulkRNA_DRAG_Figures.Rmd
├── 2.3_BulkDNA_DRAG_Figure.Rmd
├── 3.1.1_sc_LineageAnalyses.Rmd
├── 3.1.2_sc_Annotation_MutantAlv.Rmd
├── 3.1.3sc_pseudotime.Rmd
├── 3.2_sc_LineageAnalyses_Figures.Rmd
├── 4_sc_Comparison_MutantWT.Rmd
├── 5.1_scMultiome_PreProcess_Annotation.Rmd
├── 5.2_sc_Multiome_Figures.Rmd
├── 5.3_scMultiome_NMF.rmd
└── 5.4_scMultiome_Figures.Rmd
```
